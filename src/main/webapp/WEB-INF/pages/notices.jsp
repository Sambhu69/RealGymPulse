                ...
                <c:forEach items="${notices}" var="n">
                    <div class="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 backdrop-blur-xl hover:border-zinc-700 transition-all relative overflow-hidden group">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <!-- Delete button (admin only, or own notices for trainers) -->
                            <c:if test="${sessionScope.loggedUser.role == 'admin' || sessionScope.loggedUser.userId == n.authorId}">
                                <form action="${pageContext.request.contextPath}/notices" method="POST" class="flex-shrink-0" onsubmit="gpConfirm(event, 'Delete this notice permanently?', 'Delete Notice');">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="noticeId" value="${n.noticeId}">
                                    <button type="submit" class="p-2 rounded-lg text-zinc-600 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100" title="Delete Notice">
